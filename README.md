...
- 🦴 **브라우저 스켈레톤 분석**: 업로드 전 사용자 기기에서 MediaPipe Pose로 관절
  ...
  Gemini 위상 감지 → ffmpeg 모션 분석 → 고정 비율 순 폴백.
...
- 🔁 **일관성 분석 (복수 스윙)**: 같은 클럽 스윙 2~5개를 올리면 스윙별 독립 채점 후
  서버가 결정적으로 집계 — 일관성 점수(0~100), 등급은 중앙값(한 번의 좋은/나쁜 스윙에
  안 흔들림), 문제를 **습관적(60%↑) / 간헐적 / 드물지만 치명적**으로 빈도 분류.
  업로드 화면에서 "정밀 분석"과 별도 탭으로 제공.
